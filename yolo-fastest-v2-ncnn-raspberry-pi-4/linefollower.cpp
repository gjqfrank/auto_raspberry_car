...
// ==================== 慢速左转 ====================
void car_turn_left_slow() {
    set_pin(rf_bwd, 1);set_pin(rf_bwd,0);
    set_pin(lf_bwd, 0);set_pin(lf_bwd,0);
    set_pin(rb_bwd, 1);set_pin(rb_bwd,0);
    set_pin(lb_bwd, 0);set_pin(lb_bwd,0);

   
}

// ==================== 慢速右转 ====================
void car_turn_right_slow() {
    set_pin(rf_fwd, 0); set_pin(rf_bwd, 0);
    set_pin(lf_fwd, 1); set_pin(lf_bwd, 0);
    set_pin(rb_fwd, 0); set_pin(rb_bwd, 0);
    set_pin(lb_fwd, 1); set_pin(lb_bwd, 0);
...
void getLineParams(cv::Vec4i line, double &k, double &b) {
    ...
    if (x2 == x1) { k = 1e6; } else { k = (double)(y2 - y1) / (x2 - x1); }
    b = y1 - k * x1;
}
...
std::vector<cv::Vec4i> filterAndMergeLines(std::vector<cv::Vec4i> &lines) {
    ...
    for (auto &l : lines) {
        ...
        bool dup = false;
        for (auto &kb : kb_list) {
            if (fabs(k - kb.first) < 0.18 && fabs(b - kb.second) < 50) { dup = true; break; }
        }
        if (!dup) { valid.push_back(l); kb_list.emplace_back(k, b); }
    }
    return valid;
}
...
int drawLanesWithCenterLine(cv::Mat &frame, std::vector<cv::Vec4i> &lines) {
    ...
    if (hasLeft) {
        int x1 = (yLow - bl) / kl; int x2 = (yHigh - bl) / kl;
        cv::line(frame, cv::Point(x1, yLow), cv::Point(x2, yHigh), cv::Scalar(255,255,255), 2);
    }
    if (hasRight) {
        int x1 = (yLow - br) / kr; int x2 = (yHigh - br) / kr;
        cv::line(frame, cv::Point(x1, yLow), cv::Point(x2, yHigh), cv::Scalar(255,255,255), 2);
    }
...
static void draw_objects(cv::Mat& cvImg, const std::vector<TargetBox>& boxes) {
    ...
    for (size_t i = 0; i < boxes.size(); i++) {
        ...
        if (id != PERSON_CLASS_ID && id != BICYCLE_CLASS_ID && id != CAR_CLASS_ID &&
            id != MOTORBIKE_CLASS_ID && id != TRAFFIC_LIGHT_CLASS_ID && id != STOP_SIGN_CLASS_ID) continue;
...
int main(int argc, char** argv) {
    ...
    if (!cap.isOpened()) { std::cerr << "open camera fail" << std::endl; return -1; }
    ...
    while (true) {
        ...
        if (validLines.size() < 2) {
            ...
            if (abs(smooth_offset) <= dead_zone) {
                car_forward_slow();
                std::cout << "慢速直行" << std::endl;
            } else if (smooth_offset < -dead_zone) {
                car_turn_left_slow();
                std::cout << "慢速左转" << std::endl;
            } else {
                car_turn_right_slow();
                std::cout << "慢速右转" << std::endl;
            }
        }
        ...
        cv::imshow("Lane", frameCopy);
        cv::imshow("Result", frame);
        if (cv::waitKey(1) == 27) { car_stop(); break; }
    }
